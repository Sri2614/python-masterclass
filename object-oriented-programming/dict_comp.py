# Dictionary Comprehension - create dictionaries using an expression

# can replace for loops and certain lambda functions

# dictionary = {key: expression for (key,value) in iterable}

cities_in_F = {'New York': 32, 'Boston': 75, 'LA':100,'Chicago': 50}

cities_in_C = {key: round(((value-32)*(5/9))) for (key,value) in cities_in_F.items()}

# dictionary = {key: expression for (key,value) in iterable if conditions}

weather = {'New York': "snowing", 'Boston': "sunny", 'LA':"sunny",'Chicago': "cloudy"}

sunny_weather = {key: value for (key,value) in weather.items() if value != "sunny"}

# dictionary = {key: if/else for (key,value) in iterable}

cities = {'New York': 32, 'Boston': 75, 'LA':10,'Chicago': 50}

desc_cities = {key: ("WARM" if value > 50 else "COLD") for (key,value) in cities.items()}

# dictionary = {key: function(value) for (key,value) in iterable}

cities_names = {'New York': 32, 'Boston': 75, 'LA':10,'Chicago': 50}

def check_temp(value):
    if value >= 70:
        return "HOT"
    elif 69 >= value >= 40:
        return "WARM"
    else:
        return "COLD"

func_desc = {key: check_temp(value) for (key,value) in cities_names.items()}

print(func_desc)